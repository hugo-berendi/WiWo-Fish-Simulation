...
public class Boid : MonoBehaviour
{
    ...
    private void TurnAtBounds()
    {
        ...
        // Check if outside bounds
            ...
            (transform.position - boundaryCenter).sqrMagnitude > (boundaryRadius * boundaryRadius)
        )
        {
            if (!turningAround)
            {
            ...
            transform.rotation = Quaternion.Slerp(
                transform.rotation,
                targetRotation,
                Time.deltaTime * boidSettings.speed
            );
        ...
        foreach (Boid other in boidList)
        {
            ...
            if (sqrMagnitudeTemp < sqrPerceptionRange)
            {
                ...
                if (other != this)
                {
                    ...
                    neighbors.Add(
                        other,
                        (other.transform.position, velocityOther, vectorBetween, sqrMagnitudeTemp)
                    );
                }
            }
        }
        ...
        if (boidSettings.separationStrength > 0)
        {
            ...
            else
            {
                foreach (KeyValuePair<Boid, (Vector3, Vector3, Vector3, float)> item in neighbors)
                {
                    ...
                    if (
                        item.Value.Item4
                        < boidSettings.perceptionRange * boidSettings.separationStrength
                    ) // Item4 == squaredDistance
                    {
                        separationForce -= item.Value.Item3; // Item3 == vectorBetween
                    }
                }
                separationForce *= boidSettings.separationStrength;
                separationForce = Vector3.ClampMagnitude(
                    separationForce,
                    boidSettings.maxAccel / 2
                ); // Clamp separation to be much weaker than other 2 methods to avoid jitter
        ...
        if (boidSettings.alignmentStrength > 0)
        {
            ...
            else
            {
                ...
                if (boidSettings.drawDebugLines)
                {
                    Debug.DrawLine(
                        transform.position,
                        transform.position + alignmentForce,
                        Color.green
                    );
                }
            }
        ...
        if (boidSettings.cohesionStrength > 0)
        {
            ...
            else
            {
                ...
                if (boidSettings.drawDebugLines)
                {
                    Debug.DrawLine(
                        transform.position,
                        transform.position + cohesionForce,
                        Color.blue
                    );
                }
            }
